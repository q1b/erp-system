						...
						<Table.Cell>
							<DropdownMenu.Root>
								...
								<DropdownMenu.Content sideOffset={2} >
									...
									<form method="POST" action="/admin/specialization/{specialization.id}?/delete" use:enhance>
										<Button type="submit" variant="ghost" size="sm" class="w-full flex justify-between">
											Delete
										</Button>
									</form>
...
{#await data.programs then programs}
	<CreateSpecializationDialog programs={programs} bind:open data={data.specializationForm} />
{/await}