...
lectures.forEach(async (lecture, i) => {
    ...
    console.log(`${i} ${course?.name} BY ${user?.name}`);
    // if (course?.id && user?.professor?.id) {
    //     const db_lecture = await db.query.lectureTable.findFirst({
    //         where: and(eq(schema.lectureTable.courseId, course?.id), eq(schema.lectureTable.professorId, user?.professor?.id))
    //     })
    //     if (db_lecture) { 
    //     }
    // }
    
    const db_lecture = (await db.insert(schema.lectureTable).values({
        active: true,
        courseId: course?.id,
    ...
    lecture["sem-id"].forEach(async (semId) => {
        console.log(`DEFINING semesterId: ${semId} to lectureId:${db_lecture.id}  course:${lecture["course-name"]} `)
        const res = await db.insert(schema.semesterLectureTable).values({
            lectureId: db_lecture.id,
            semesterId: semId,
        }).returning();
        console.dir(res.at(0), { depth: 3 })
    })
});
